...
由于第三方音源可能涉及用户个人配置，脚本实行隔离存放：

- **分类保存**：如果你在登录账号后上传了对应的 JS 脚本源，文件会被独立保存在 `data/users/source/{用户名}` 文件夹下。如果没有登录而直接上传，源文件会被放到公共目录 `source/_open` 中。
- **列表加载**：前端获取源列表时，会合并读取“公开目录”下的所有源，加上“你所登录的专属目录”下的源，共同组成你的最终可用音源列表。
...
- **第一级 (文件缓存)**：如果在设置里开启了“缓存歌曲文件”，音频会在第一次播放时下载并保存在服务器的 `cache` 物理文件夹内。第二次播放同一首歌时，服务端直接把这首本地文件传输给前端（支持 206 Partial Content 断点拉取），可以随时无延迟拖拽进度条。
